...
create table `participation` (`meetup_id` integer not null, `user_id` integer not null, primary key (`meetup_id`, `user_id`));
...
alter table `meetups` add column `image_id` integer null;
alter table `meetups` add column `organizer_id` integer null;
...
alter table `agenda_items` add column `meetup_id` integer null;
create index `agenda_items_meetup_id_index` on `agenda_items` (`meetup_id`);

alter table `images` add column `user_id` integer null;
